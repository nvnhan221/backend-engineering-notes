# Backend Engineering Notes

Welcome to the Backend Engineering Notes documentation!

This documentation site is built using [MkDocs Material](https://squidfunk.github.io/mkdocs-material/), following the **Docs-as-code** approach.

## What is Docs-as-code?

Docs-as-code is a documentation methodology that treats documentation like source code:

- ✅ Version controlled with Git
- ✅ Written in Markdown
- ✅ Built and deployed automatically
- ✅ Collaborative and reviewable
- ✅ Easy to maintain and update

## Quick Links

- [Foundations](foundations/index.md) - Core concepts and principles
- [Messaging](messaging/index.md) - Message queues and event-driven patterns
- [Examples](examples/index.md) - Practical code examples
- [Microservices](microservices/index.md) - Microservices architecture
- [Data](data/index.md) - Database and data management
- [Observability](observability/index.md) - Monitoring, logging, and tracing
- [Reliability](reliability/index.md) - Building resilient systems
- [Security](security/index.md) - Security best practices
- [ADR](adr/index.md) - Architecture Decision Records
- [Getting Started](getting-started/installation.md) - Set up your environment

## Features

This documentation site includes:

- 📚 **Material Design** - Beautiful, responsive UI
- 🔍 **Full-text Search** - Find content quickly
- 🌓 **Dark Mode** - Easy on the eyes
- 📱 **Mobile Friendly** - Works on all devices
- 🔗 **Version Control** - Track changes over time
- 🚀 **Static Site** - Fast and reliable hosting

## Contributing

Contributions are welcome! Please read our contributing guidelines and submit pull requests.

---

*Last updated: {{ git_revision_date_localized }}*
